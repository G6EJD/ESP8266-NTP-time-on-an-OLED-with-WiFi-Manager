...
void loop() {
  if (seconds > 59) {
    ...
    epoch   = timeClient.getEpochTime()+405*24*60*60 - 0*60*60 - 0*60; // You can test moving time forward or back by adding or subtracting the required number of seconds 1-day = 24*60*60
    seconds = epoch % 60;
  }
...
void display_time(){ // Note Ticker called routines cannot get a time update using timeclient.update()!
  ...
  if ((current_month  > 2 &&  current_month < 9 )   || 
      (current_month == 2 && (current_day-DoW) >= 25 && DoW >  0) ||
      (current_month == 2 && (current_day-DoW) >= 25 && DoW == 0 && UTC_hours >= 1) 
      && DST == false) DST = true;    // DST starts 2nd Sunday of March at 01:00
  // Turn DST OFF
  if ((current_month  < 2 &&  current_month > 9 ) || 
      (current_month == 9 && (current_day-DoW) <= 25 && DoW > 0 )  ||
      (current_month == 9 && (current_day-DoW) <= 25 && (DoW == 0 && (UTC_hours >= 1) && UTC_hours < 2) )
      && DST == true) DST = false;  // DST ends 1st Sunday of November at 02:00
  ...
  display.println(UTC_hours);
  display.setCursor(19,32); // Move down to a position that can accomodate seconds display
  display.print((seconds < 10 || seconds%60==0) ? "(0" + String(seconds%60)+")" : "("+String(seconds%60)+")");
