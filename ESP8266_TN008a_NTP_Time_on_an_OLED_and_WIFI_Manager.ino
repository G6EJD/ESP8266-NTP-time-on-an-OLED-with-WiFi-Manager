...
bool DST = false; // DayLightSaving indicator
...
NTPClient timeClient(time_udp,"time.nist.gov", 0, 60000);
...
void display_time(){ // Note Ticker called routines cannot update the time!
  ...
  display.setCursor(20,32); // Move down to a position that can accomodate seconds display  seconds = seconds + 1;    // Increase the seconds count
