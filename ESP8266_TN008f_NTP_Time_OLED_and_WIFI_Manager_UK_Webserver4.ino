...
#include <Adafruit_SSD1306.h> // Must the version via the WeMos site under OLED - not the adafruit_ssd1306 version...
...
void display_time(){ // Note Ticker called routines cannot get a time update using timeclient.update()!
  ...
  if (DST == true) epoch = epoch + 3600; else local_epoch = epoch;
  epoch         = epoch + 1;
  current_year  = 1970+epoch/(365.25*24*60*60);
  current_month = int(((1970+epoch/(365.25*24*60*60)-current_year)*12))%12; // Months are numbered 0-11 where 0 = Jan
  current_day   = (epoch + int(TimeZone * 3600)) / 86400;
  hours         = (epoch + int(TimeZone * 3600)) % 86400 / 3600;
  UTC_hours     = (epoch + int(TimeZone * 3600)) % 86400 / 3600;
  minutes       = (epoch + int(TimeZone * 3600)) % 3600 / 60;
  seconds       = (epoch + int(TimeZone * 3600)) % 60;
